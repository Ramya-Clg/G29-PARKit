...
//       // Find any overlapping reservations for this slot
...
//       // If no overlapping reservation found, this slot is available
...
//     // Check if this vehicle already has a reservation for this time
...
// // Checkout endpoint remains the same
...
parkingSlotRouter.post("/reserve", authorizationMiddleware, async (req, res) => {
  try {
    ...
    if (!req.user?._id || !vehicleNumberPlate) {
      return res.status(400).json({
        success: false,
        msg: !req.user?._id ? "User not authenticated" : "Vehicle number plate is required"
      });
    }
