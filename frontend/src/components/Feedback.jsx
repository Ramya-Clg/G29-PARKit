...
export function Feedback() {
  ...
  return (
    <Card className="w-full max-w-[calc(100%-2rem)] sm:max-w-md mx-auto my-8">
      <CardHeader>
        <CardTitle className="text-2xl sm:text-3xl text-center">Feedback Form</CardTitle>
        <CardDescription className="text-center">We value your feedback. Please fill out the form below.</CardDescription>
      </CardHeader>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <CardContent className="space-y-4">
            ...
            >
              {[1, 2, 3, 4, 5].map((value) => (
                <div key={value} className="flex items-center space-x-2">
                  <RadioGroupItem className="border-2" value={value.toString()} id={`rating-${value}`} />
                  <Label htmlFor={`rating-${value}`} className="text-sm sm:text-base">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="message" {...form.register('message')} className="w-full min-h-[100px]" />
